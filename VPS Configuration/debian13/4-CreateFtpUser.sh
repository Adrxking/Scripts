...
while true; do
    read -r -p "Ingrese el nombre del usuario FTP (o 'salir' para terminar): " usuario
    if [ "$usuario" = "salir" ]; then
        break
    fi

    read -r -p "Ingrese el path ABSOLUTO donde este usuario iniciará (ej. /var/www/wordpress): " ftp_path
    ...
    if [ ! -d "$ftp_path" ]; then
        ...
        if [[ "$respuesta" =~ ^[Ss] ]]; then
            mkdir -p "$ftp_path"
            # El directorio raíz del chroot debe pertenecer a root.
    ...
    if ! useradd -d "$ftp_path" -s /usr/sbin/nologin "$usuario"; then
